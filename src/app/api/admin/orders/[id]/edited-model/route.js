...
export async function POST(request, ctx) {
  ...
  const limited = limitByKey("edited-model", guard.user.id, 20, 60_000);
  ...
  if (!statusDef?.clientCanEditModel) {
    return NextResponse.json(
      { error: "Solo puedes reemplazar el modelo cuando el estado permite editar." },
      { status: 403 },
    );
  }
  ...
  const relative = `models/${order.draftId}/order-${order.id}-edited.glb`;
  await saveBuffer(relative, buffer);

  await db
    .update(orders)
    .set({ editedModelPath: relative, updatedAt: new Date() })
    .where(eq(orders.id, order.id));

  await db.insert(orderEvents).values({
    orderId: order.id,
    status: order.status,
    note: "Modelo editado adjuntado",
  });
