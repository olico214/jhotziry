...
- **Imagen → 3D**: solo admin, `POST /api/admin/orders/[id]/generate` → Tripo `image-to-model` → `drafts.model_path` (`.glb` en disco) → descarga vía `GET /api/files/[id]` (solo admin). El admin lo ve en un **visor 3D** en `/mis-pedidos/[id]` (`AdminModelViewer` + `ViewerCanvas`: órbita, auto-rotar, modos **Texturizado**/**Wireframe**); **Arcilla** y **Resina** (imprimir sin color) aparecen como "Pronto" (deshabilitados). El cliente no ve el modelo.
