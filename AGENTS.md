...
`users` (email, fullName, address, isAdmin, credits), `sessions` (+`csrf_hash`), `magic_links`, `invitations`, `access_requests`, `drafts` (mode, style, prompt, enhancedPrompt, aiSummary, `sourceImage`/`previewImage` jsonb, modelPath, modelPartsPath, status), `generation_jobs` (stage, status, progress, attempts, providerJobId), `orders`, `order_events` (timeline), `order_messages` (chat), `credit_transactions`, `posts` (tags jsonb), `post_comments`, `post_likes`, `quote_requests`, `notifications`, `order_statuses` (key, label, sortOrder, `client_can_edit_model`, `client_uploads_photo`, `client_photo_next_status`, active, isSystem).
...
- **Imagen → 3D**: solo admin, `POST /api/admin/orders/[id]/generate` → Tripo `image-to-model` → `drafts.model_path` (`.glb` en disco). Al terminar, el cliente recibe **correo + notificación** y puede **ver el modelo en 3D (solo vista, sin descarga)** en `/mis-pedidos/[id]`; el **admin sí lo descarga**. Visor `Model3DViewer` + `ViewerCanvas`: órbita, **zoom (botones + slider)**, **pantalla completa**, auto-rotar, reiniciar, modos **Texturizado**/**Wireframe** (y **Arcilla**/**Resina** "Pronto") y **separar/mostrar-ocultar partes** cuando el modelo trae varias. El modelo **por partes** (sin textura) llega sin color (Tripo `generate_parts` es incompatible con textura): se ve tipo arcilla, y admin y cliente pueden **pintar cada parte** con una paleta de filamentos; el **admin** puede **descargar el render en PNG** y el **GLB coloreado** (`GLTFExporter`). El admin puede generar **texturizado**, **por partes** o **ambos** (`mode` = `textured` | `parts` | `both`): "partes" usa `generate_parts` sin textura; "ambos" crea **dos trabajos** y guarda **dos archivos** (`drafts.model_path` texturizado y `drafts.model_parts_path` sin color; también en `orders`). Coste Tripo (H2/H3): **texturizado ≈30** créditos, **por partes ≈40** (20 sin textura + 20 partes), **ambos ≈70**; se muestran con `NEXT_PUBLIC_TRIPO_COST_MODEL`/`NEXT_PUBLIC_TRIPO_COST_PARTS`. `GET /api/files/[id]?kind=parts` sirve el modelo por partes. `GET /api/files/[id]` permite **dueño o admin** (servido inline; el admin descarga con el atributo `download`). El **export GLB** del visor usa una copia **limpia** (todas las partes visibles, sin separación, con los colores pintados); el **PNG** lo puede descargar cualquiera con sesión y los **GLB** solo el admin. El admin puede **guardar el GLB coloreado en el servidor** (`POST /api/admin/orders/[id]/color-model` → `orders.color_model_path`) y **subir un GLB editado a mano** (`POST /api/admin/orders/[id]/edited-model` → `orders.edited_model_path`, solo si el estado permite editar); se sirven con `GET /api/orders/[id]/model?kind=color|edited` (color solo admin; edited dueño/admin).
- **Prompts para filamento**: `deepseek.js` (texto→imagen) y `provider.js` (imagen→imagen) instruyen **paletas básicas de 2-4 colores planos**, sin degradados ni detalles finos, y diseños por **regiones de color** para poder imprimir en partes.
- **Trabajos**: `src/lib/jobs/runner.js` (`ensureRunner`) avanza los `generation_jobs` en `processing` cada 5 s **sin solaparse**, sin depender del navegador; se arranca al inicio del servidor vía `src/instrumentation.js`. Las **programaciones de estado** se procesan en un **intervalo aparte cada 15 s** (`processScheduledStatuses`, reclamo atómico del pedido y envío de avisos con concurrencia limitada). Al terminar una generación se envía correo y notificación. Cada cambio de estado avisa al cliente por correo + notificación (`notifyOrderStatusChange`).
...
- **Notificaciones**: tabla `notifications` + campana en el header (`/api/notifications`, `/api/notifications/read`). Se crean al recibir un **pedido nuevo** (al admin), un **mensaje de pedido** (a la otra parte) y **publicaciones/comentarios**. Cada una enlaza al recurso concreto: pedidos → `/mis-pedidos/[id]` (mensajes → `/mis-pedidos/[id]#chat`), blog → `/blog#post-[id]`. `GET /api/notifications` devuelve **solo las no leídas** (al marcarlas dejan de aparecer).
- **Estado y edición del 3D**: los estados viven en la tabla `order_statuses` (se autosiembra en `src/lib/orders/statuses.js`, los del sistema no se borran) y se gestionan en la pestaña **Estados** del admin (`StatusManager`), donde se **reordenan con drag and drop** (`POST /api/admin/order-statuses/reorder`). Cada estado tiene `clientCanEditModel`: si está marcado (p. ej. **Modelo listo**), el cliente puede **pintar/cambiar los colores** de las partes; en estados como **Confirmado por cliente** el pintado queda bloqueado (girar, zoom y ver siempre están disponibles; `Model3DViewer` lo recibe como `canEdit`). El admin siempre puede pintar, interactuar y descargar. Los colores elegidos se **guardan en `orders.part_colors`** (auto-guardado desde `Model3DViewer` vía `PATCH /api/orders/[id]`, que rechaza al cliente si el estado no lo permite); al recargar se restauran. Cada estado puede además marcar `clientUploadsPhoto` (p. ej. **Recibido**): el cliente sube una **foto de recibido** (`POST /api/orders/[id]/photo`) y el pedido avanza solo al `clientPhotoNextStatus` configurado (p. ej. **Completado**). El admin puede **programar** un cambio de estado a una fecha/hora (`POST /api/admin/orders/[id]/schedule` → `orders.scheduled_status`/`scheduled_status_at`); el runner (`processScheduledStatuses`) lo aplica al vencer y lo registra en el timeline. `OrdersBoard` y `AdminOrderActions` toman las opciones/labels de esa tabla.
...
Con sesión + CSRF (mutaciones) o solo sesión (lecturas): `orders`, `orders/[id]` (PATCH colores del 3D), `orders/[id]/model` (GET modelos coloreado/editado), `orders/[id]/photo` (GET/POST foto de recibido), `orders/[id]/messages`, `drafts`, `drafts/claim`, `posts` (POST), `posts/[id]/comments`, `posts/[id]/like`, `notifications`, `notifications/read`, `generate/text`, `generate/image`, `assistant/expand`, `quote`, `auth/logout`.
Solo admin: `admin/credits`, `admin/invitations`, `admin/access-requests/[id]/invite`, `admin/orders/[id]`, `admin/orders/[id]/generate`, `admin/orders/[id]/color-model`, `admin/orders/[id]/edited-model`, `admin/orders/[id]/schedule`, `admin/order-statuses`, `admin/order-statuses/reorder`, `admin/order-statuses/[id]`, `admin/posts/[id]`, `GET /api/files/[id]`.
