...
- **Estado y edición del 3D**: los estados viven en la tabla `order_statuses` (se autosiembra en `src/lib/orders/statuses.js`, los del sistema no se borran) y se gestionan en la pestaña **Estados** del admin (`StatusManager`). Cada estado tiene `clientCanEditModel`: si está marcado (p. ej. **Modelo listo**), el cliente puede **pintar/cambiar los colores** de las partes; en estados como **Confirmado por cliente** el pintado queda bloqueado (girar, zoom y ver siempre están disponibles; `Model3DViewer` lo recibe como `canEdit`). El admin siempre puede pintar, interactuar y descargar. Los colores elegidos se **guardan en `orders.part_colors`** (auto-guardado desde `Model3DViewer` vía `PATCH /api/orders/[id]`, que rechaza al cliente si el estado no lo permite); al recargar se restauran. `OrdersBoard` y `AdminOrderActions` toman las opciones/labels de esa tabla.
...
Con sesión + CSRF (mutaciones) o solo sesión (lecturas): `orders`, `orders/[id]` (PATCH colores del 3D), `orders/[id]/model` (GET modelos coloreado/editado), `orders/[id]/messages`, `drafts`, `drafts/claim`, `posts` (POST), `posts/[id]/comments`, `posts/[id]/like`, `notifications`, `notifications/read`, `generate/text`, `generate/image`, `assistant/expand`, `quote`, `auth/logout`.
Solo admin: `admin/credits`, `admin/invitations`, `admin/access-requests/[id]/invite`, `admin/orders/[id]`, `admin/orders/[id]/generate`, `admin/orders/[id]/color-model`, `admin/orders/[id]/edited-model`, `admin/order-statuses`, `admin/order-statuses/[id]`, `admin/posts/[id]`, `GET /api/files/[id]`.
